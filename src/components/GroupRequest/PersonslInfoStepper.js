...
const PersonalInfoStepper = forwardRef((props, ref) => {
  ...
  const handleSubmit = () => {
    ...
    if (isVilid != true) {
      ...
      var personalInfo = counter.personalInfoReducer.filter(
        (item) => item.applicantNumber == props.applicantNumber
      )[personalInfoLength - 1];
      const addressLength = counter.address.filter(
        (item) => item.applicantNumber == props.applicantNumber
      ).length;
      var addressInfo = counter.address.filter(
        (item) => item.applicantNumber == props.applicantNumber
      )[addressLength - 1];
      const travelPlanLength = counter.travelPlan.filter(
        (item) => item.applicantNumber == props.applicantNumber
      ).length;
      ...
      if (props.applicantNumber === 1) {
        ...
        API.post(
          ...
          .then((todo) => {
            ...
            const commonData = {
              requestPersonId:
                todo.data.serviceResponseList[0].personResponses
                  .requestPersonId,
            };
        ...
        let requestBody = {
          ...
          appointmentIds: appointment
            ? [appointment[props.applicantNumber - 1].id]
            : [],
          applicants: [
            {
              personId: 0,
        ...
        API.post(
          'https://epassportservices.azurewebsites.net/Request/api/V1.0/Request/AddPerson',
          requestBody,
          config
        )
          .then((todo) => {
  ...
  function getStepContent(stepIndex) {
    switch (stepIndex) {
      ...
      case 1:
        return (
          <AddressInfo ref={childRef} applicantNumber={props.applicantNumber} />
        );
      case 2:
        return (
          <FamilyInformation
