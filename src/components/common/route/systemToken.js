  ...
  useEffect(() => {
    axios({
      ...
      axios({
        ...
        .then((todo) => {
          localStorage.setItem('nationalitys', JSON.stringify(todo.data.nationalitys));
    ...
    API.get(
        ...
        .then((todo) => {
          localStorage.setItem('occupations', JSON.stringify(todo.data.occupations));
        })
        .catch((err) => {
            console.log('AXIOS ERROR: ', err.response);
        });
        axios({
          ...
          .then(async (response) => {
            localStorage.setItem('familyTypesResponse', JSON.stringify(response.data.familyTypesResponse));
          })
          .catch((error) => {
